...
#### My Idea: 
 ...
 输入输出范例：
  ...
  演示一下在给定输入 nums = [1,3,-1,-3,5,3,6,7] 和 k = 3 的情况下，`MaxSlidingWindow` 方法中 `myDequeue` 对象的状态变化。
...
public class Solution
{
    public int[] MaxSlidingWindow(int[] nums, int k)
    {
        ...
        Console.WriteLine();
        //输入 nums = [1,3,-1,-3,5,3,6,7]，k = 3, 这里的打印结果为：
        // i = 0， window.Enqueue(nums[i])，即 window.Enqueue(1)，此时 window.linkedList = [1]
        // i = 1， window.Enqueue(nums[i])，即 window.Enqueue(3)，remove 比 3 小的元素，此时 window.linkedList = [3]
        // i = 2， window.Enqueue(nums[i])，即 window.Enqueue(-1)，remove 比 -1 小的元素，此时 window.linkedList = [3, -1]
        // i = 3， window.Enqueue(nums[i])，即 window.Enqueue(-3)，remove 比 -3 小的元素，此时 window.linkedList = [3, -1, -3]
        // i = 4， window.Enqueue(nums[i])，即 window.Enqueue(5)，remove 比 5 小的元素，此时 window.linkedList = [5]
        // i = 5， window.Enqueue(nums[i])，即 window.Enqueue(3)，remove 比 3 小的元素，此时 window.linkedList = [5, 3]
        // i = 6， window.Enqueue(nums[i])，即 window.Enqueue(6)，remove 比 6 小的元素，此时 window.linkedList = [6]
        // i = 7， window.Enqueue(nums[i])，即 window.Enqueue(7)，remove 比 7 小的元素，此时 window.linkedList = [7]
        
        
        // window.Max()，即 window.linkedList.First.Value，即 7
